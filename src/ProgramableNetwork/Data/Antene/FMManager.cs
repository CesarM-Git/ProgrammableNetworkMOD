		...
		public Dictionary<int, (Fix32 signalStrength, FMDataBandChannel channelInfo)> Signals(Tile3i position, int? channelIdx = null, bool logging = false)
		{
			...
			foreach ((Tile3i tile, Antena antenna) in m_antenas)
			{
				...
				if (channelIdx != null)
				{
					...
					if (logging) {
						Log.Info($"[FMManager] IN distance [{channelIndex}]: {distances[channelIndex]} with strength: {channels[channelIndex].Item1} and datalen: {channels[channelIndex].Item2.ValueLength}");
					}
					continue;
				}
