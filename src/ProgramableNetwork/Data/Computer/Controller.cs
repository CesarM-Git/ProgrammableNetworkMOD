		...
		IUnityConsumingEntity, IComputingConsumingEntity, IElectricityConsumingEntity, IMaintainedEntity, IObjectWithCustomTitle
	{
		...
		private void initContexts(int saveVersion, DependencyResolver resolver)
		{
			...
			else
			{
				...
				// One-time migration: saves written before CONTROLLER_MODULE_ID_POOL
				// may have duplicate Module.Id values across controllers (from blueprint
				// copy-paste).  Regenerate every ID via the global ModuleIdManager and
				// remap InputModules connections.  Once saved at the new version, this
				// block is skipped on future loads.
				if (m_controllerLoadedVersion < CONTROLLER_MODULE_ID_POOL)
				{
					Log.Info($"Controller {Id}: migrating module IDs (save version {m_controllerLoadedVersion} < {CONTROLLER_MODULE_ID_POOL})");
